use std::{alloc::Layout, sync::atomic::AtomicBool};

use super::{cell::GcPointerBase, Heap};
...
impl PreciseAllocation {
    ...
    pub fn from_cell(ptr: *mut GcPointerBase) -> *mut Self {
        unsafe {
            ptr.cast::<u8>()
    ...
    pub fn cell(&self) -> *mut GcPointerBase {
        let addr = unsafe { (self as *const Self as *mut u8).add(Self::header_size()) };
        addr.cast()
    }
    ...
    pub fn try_create(size: usize, heap: *mut Heap) -> *mut Self {
        let adjusted_alignment_allocation_size = Self::header_size() + size + Self::HALF_ALIGNMENT;
        unsafe {
            let mut space = std::alloc::alloc(
