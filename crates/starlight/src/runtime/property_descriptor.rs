...
use super::{attributes::AttrExternal, value::JsValue};
...
impl StoredSlot {
    ...
    pub fn accessor(&self) -> Gc<Accessor> {
        assert!(self.attributes.is_accessor());
        unsafe { self.value.as_cell().downcast_unchecked() }
    }
    ...
    pub fn merge(&mut self, context: &mut VirtualMachine, desc: &PropertyDescriptor) {
    ...
    pub fn new(context: &mut VirtualMachine, desc: &PropertyDescriptor) -> Self {
        let mut this = Self {
        ...
        if desc.is_data() {
            ...
            this.value = JsValue::new(accessor);
